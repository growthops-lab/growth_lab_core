...
const intervalSeconds = Math.max(10, Number(process.env.SNS_POST_WORKER_INTERVAL_SECONDS ?? "60"));
const maxPerTick = Math.max(1, Number(process.env.SNS_POST_WORKER_MAX_PER_TICK ?? "3"));
const workerEnabled = process.env.SNS_POST_WORKER_ENABLED === "true";

async function tick() {
  const due = await prisma.socialPostQueue.findMany({
    where: {
      queueStatus: { in: [SocialPostQueueStatus.READY, SocialPostQueueStatus.SCHEDULED] },
  ...
  for (const item of due) {
    ...
    console.log(`[social-post-worker] queue=${item.id} status=${result.status}`);
  }
}
