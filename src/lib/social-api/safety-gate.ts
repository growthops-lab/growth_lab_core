...
export async function runSocialPostSafetyGate(queueId: string) {
  ...
  if (queue.approvalStatus !== SocialPostApprovalStatus.APPROVED)
    reasons.push("Human approval is required before posting.");
  if (queue.rateLimitStatus !== SocialRateLimitStatus.OK)
    reasons.push("Rate limit status is not OK.");

  const linkCheckStatus = queue.linkUrl
    ? SocialPostCheckStatus.PASSED
    : SocialPostCheckStatus.WARNING;
  if (!queue.linkUrl)
    reasons.push(
      "No destination link was found; manual review is recommended.",
    );
  ...
  const manualReviewRequired =
    blocked ||
    risk.status === SocialPostCheckStatus.WARNING ||
    duplicate.status === SocialPostCheckStatus.WARNING;
  ...
  await prisma.socialPostQueue.update({
    ...
    data: {
      ...
      riskCheckStatus: risk.status,
      dedupCheckStatus: duplicate.status,
      manualReviewStatus: manualReviewRequired
        ? "OPEN"
        : queue.manualReviewStatus,
