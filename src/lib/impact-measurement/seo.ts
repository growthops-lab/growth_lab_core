...
function daysBetween(start: Date, end: Date) {
  return Math.max(0, Math.floor((end.getTime() - start.getTime()) / (24 * 60 * 60 * 1000)) + 1);
}
...
function verdict(beforeClicks: number, afterClicks: number, beforeImpressions: number, afterImpressions: number) {
  if (afterClicks > beforeClicks && afterImpressions >= beforeImpressions * 0.8) return SeoImpactVerdict.IMPROVED;
  if (afterClicks < beforeClicks * 0.8 && afterImpressions < beforeImpressions * 0.8) return SeoImpactVerdict.DECLINED;
  if (afterClicks === beforeClicks && afterImpressions === beforeImpressions) return SeoImpactVerdict.NO_CHANGE;
  return SeoImpactVerdict.MIXED;
}

export async function createSeoImpactMeasurement(prisma: PrismaClient, articleImprovementTaskId: string) {
  const task = await prisma.articleImprovementTask.findUniqueOrThrow({ where: { id: articleImprovementTaskId } });
  const now = new Date();
  ...
  const afterStart = latestDraftUpdate ? new Date(latestDraftUpdate.createdAt) : new Date(now);
  ...
  const [beforeGsc, afterGsc, beforeGa4, afterGa4, beforeRevenue, afterRevenue] = await Promise.all([
    prisma.searchConsolePageDaily.aggregate({
      where: { mediaId: task.mediaId, wordpressPostId: task.wordpressPostId, date: { gte: beforeStart, lte: beforeEnd } },
      _sum: { clicks: true, impressions: true },
      _avg: { position: true }
    }),
    prisma.searchConsolePageDaily.aggregate({
      where: { mediaId: task.mediaId, wordpressPostId: task.wordpressPostId, date: { gte: afterStart, lte: afterEnd } },
      _sum: { clicks: true, impressions: true },
      _avg: { position: true }
    }),
    ...
    prisma.revenueEvent.aggregate({
      where: { mediaId: task.mediaId, wordpressPostId: task.wordpressPostId, eventDate: { gte: beforeStart, lte: beforeEnd }, status: "APPROVED" },
      _sum: { approvedReward: true }
    }),
    prisma.revenueEvent.aggregate({
      where: { mediaId: task.mediaId, wordpressPostId: task.wordpressPostId, eventDate: { gte: afterStart, lte: afterEnd }, status: "APPROVED" },
      _sum: { approvedReward: true }
    })
  ]);
  ...
  const afterEnoughDays = Boolean(latestDraftUpdate) && daysBetween(afterStart, afterEnd) >= minAfterDays;
  ...
  const dataConfidence =
    !afterEnoughDays || afterClicks + beforeClicks < minClicks || afterImpressions + beforeImpressions < minImpressions
      ? DataConfidence.LOW
      : DataConfidence.MEDIUM;
  const status = !afterEnoughDays ? SeoImpactStatus.INSUFFICIENT_DATA : SeoImpactStatus.COMPLETED;
  const measurement = await prisma.seoImpactMeasurement.create({
    data: {
      ...
      status,
      verdict: status === SeoImpactStatus.INSUFFICIENT_DATA ? SeoImpactVerdict.INSUFFICIENT_DATA : verdict(beforeClicks, afterClicks, beforeImpressions, afterImpressions),
      beforeStart,
      beforeEnd,
      afterStart,
      afterEnd,
      ...
      dataConfidence,
      summary: status === SeoImpactStatus.INSUFFICIENT_DATA ? "After period is not long enough for reliable impact measurement." : "SEO impact measurement calculated."
    }
  });
  await prisma.articleImprovementTask.update({
    where: { id: task.id },
    data: { status: status === SeoImpactStatus.INSUFFICIENT_DATA ? ArticleImprovementStatus.MEASURING : ArticleImprovementStatus.DONE }
  });
