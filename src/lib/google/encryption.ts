...
function getKey() {
  ...
  if (!secret) {
    throw new Error("GOOGLE_TOKEN_ENCRYPTION_KEY is required for real Google OAuth tokens.");
  }
...
export function decryptGoogleToken(value: string) {
  ...
  if (!iv || !tag || !encrypted) throw new Error("Invalid encrypted Google token format.");
