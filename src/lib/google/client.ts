...
function requestTimeoutMs(apiName: GoogleApiName) {
  if (apiName === GoogleApiName.GA4)
    return Number(process.env.GA4_REQUEST_TIMEOUT_MS ?? 20000);
  if (apiName === GoogleApiName.SEARCH_CONSOLE)
    return Number(
  ...
  return withRetry(
    async () => {
      ...
      const timeout = setTimeout(
        () => controller.abort(),
        requestTimeoutMs(input.apiName),
      );
      ...
      if (!response.ok) {
        const errorCode =
          payload?.error?.status ?? payload?.error?.errors?.[0]?.reason;
