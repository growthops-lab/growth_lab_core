...
export async function calculateCampaignGrowthScore(campaignId: string) {
  ...
  const revenueScore = Math.min(20, Math.floor(Number(latestRoi?.approvedRevenue ?? 0) / 500));
  const roiScore = Math.max(0, Math.min(20, Math.round((latestRoi?.roiApproved ?? 0) * 20)));
  const riskScore = Math.max(0, 20 - risks * 5);
  const totalScore = contentScore + 10 + 10 + revenueScore + roiScore + 10 + riskScore;
  ...
  const snapshot = await prisma.campaignGrowthScoreSnapshot.create({
    data: {
      ...
      reasoning: "Phase 10 MVP score uses local campaign items, ROI snapshots, and open risks. It never changes budget or posting volume automatically."
    }
  });
