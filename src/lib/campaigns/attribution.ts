import { AttributionSourceType, CampaignRiskSeverity, CampaignRiskType, DataConfidence } from "@prisma/client";
...
export async function attributeRevenueToCampaign(input: {
  ...
  if (allocationRate <= 0 || allocationRate > 1) {
    throw new Error("Campaign attribution allocationRate must be greater than 0 and at most 1.");
  }
  ...
  const existingRate = existingAttributions.reduce((sum, item) => sum + item.allocationRate, 0);
  ...
  if (effectiveAllocationRate <= 0) {
    await createDoubleCountRisk(input.campaignId, input.sourceType, input.sourceId, existingRate, allocationRate);
    throw new Error("Campaign attribution would exceed total allocationRate of 1.0 for the same source.");
  }
  ...
  return prisma.campaignRevenueAttribution.upsert({
    ...
    create: {
      ...
      dataConfidence: effectiveAllocationRate < allocationRate ? DataConfidence.INSUFFICIENT : DataConfidence.LOW,
      doubleCountProtected: process.env.CAMPAIGN_ROI_PREVENT_DOUBLE_COUNT === "true",
      notes:
