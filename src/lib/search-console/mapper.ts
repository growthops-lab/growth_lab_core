...
export function normalizedSearchType(value: string | undefined) {
  ...
  return Object.values(SearchType).includes(normalized as SearchType)
...
export function normalizedSearchDevice(value: string | undefined) {
  const normalized = String(value || "UNKNOWN")
    ...
    .toUpperCase()
    .replace(/[\s-]+/g, "_");
  return Object.values(SearchDevice).includes(normalized as SearchDevice)
    ? (normalized as SearchDevice)
    : SearchDevice.UNKNOWN;
}
