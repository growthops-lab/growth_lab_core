...
export async function fetchAllSearchConsoleRows(
  ...
  const rowLimit = Math.min(Number(process.env.GOOGLE_SEARCH_CONSOLE_ROW_LIMIT ?? 25000), 25000);
  const maxPages = Number(process.env.GOOGLE_SEARCH_CONSOLE_MAX_PAGES_PER_SYNC ?? 10);
