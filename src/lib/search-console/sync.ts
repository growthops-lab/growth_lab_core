...
import { createSearchConsoleRequest, searchConsoleEndpoint } from "@/src/lib/search-console/query";
import { gscMetrics, normalizedSearchDevice, normalizedSearchType } from "@/src/lib/search-console/mapper";
...
async function loadPosts(prisma: PrismaClient, mediaId: string) {
  return prisma.wordPressPost.findMany({ where: { mediaId }, select: { id: true, slug: true, wordpressPostUrl: true } });
}

function matchPost(posts: Awaited<ReturnType<typeof loadPosts>>, pageUrl: string) {
  return posts.find((post) => pageUrl.toLowerCase().includes(post.slug.toLowerCase())) ?? null;
}

export async function syncSearchConsoleQueryPageDaily(
  prisma: PrismaClient,
  input: { searchConsolePropertyId: string; periodStart: Date; periodEnd: Date; googleSyncRunId?: string | null }
) {
  const property = await prisma.searchConsoleProperty.findUniqueOrThrow({ where: { id: input.searchConsolePropertyId } });
  if (!property.googleConnectionId) throw new Error("Search Console property has no Google connection.");
  ...
  const responses = await fetchAllSearchConsoleRows(prisma, {
    ...
    createBody: (rowLimit, startRow) =>
      createSearchConsoleRequest(dateString(input.periodStart), dateString(input.periodEnd), ["date", "query", "page", "country", "device", "searchAppearance"], rowLimit, startRow)
  });
  ...
  for (const response of responses) {
    for (const row of response.rows ?? []) {
      ...
      await prisma.searchConsoleQueryPageDaily.upsert({
        ...
        update: { wordpressPostId: post?.id, ...metrics, dataConfidence: DataConfidence.HIGH },
        create: {
          mediaId: property.mediaId,
...
export async function syncSearchConsoleQueryDaily(
  ...
  const responses = await fetchAllSearchConsoleRows(prisma, {
    ...
    requestType: RequestType.SEARCH_CONSOLE_API_SYNC,
    createBody: (rowLimit, startRow) =>
      createSearchConsoleRequest(dateString(input.periodStart), dateString(input.periodEnd), ["date", "query", "country", "device"], rowLimit, startRow)
  });
  let savedRows = 0;
  for (const response of responses) {
    for (const row of response.rows ?? []) {
  ...
  if (input.googleSyncRunId) {
    await prisma.googleSyncRun.update({ where: { id: input.googleSyncRunId }, data: { savedRows, requestedRows: savedRows, apiCalls: responses.length, status: GoogleSyncRunStatus.SUCCESS, finishedAt: new Date() } });
  }
  return { savedRows, apiCalls: responses.length };
}
...
export async function syncSearchConsolePageDaily(
  ...
  const responses = await fetchAllSearchConsoleRows(prisma, {
    ...
    requestType: RequestType.SEARCH_CONSOLE_API_SYNC,
    createBody: (rowLimit, startRow) =>
      createSearchConsoleRequest(dateString(input.periodStart), dateString(input.periodEnd), ["date", "page"], rowLimit, startRow)
  });
  const posts = await loadPosts(prisma, property.mediaId);
  let savedRows = 0;
  for (const response of responses) {
