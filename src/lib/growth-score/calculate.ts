...
import {
  ...
  recommendationTitle,
} from "@/src/lib/growth-score/recommendation";
...
export async function calculateGrowthScore(
  ...
  const activeDays = new Set(
    trafficDays.map((item) => item.metricDate.toISOString().slice(0, 10)),
  ).size;
  const trafficTotal = trafficDays.reduce(
    (sum, item) => sum + item.impressions,
    0,
  );
  const trafficScore = clampScore(Math.min(trafficTotal / 100, 100));
  const revenueScore = clampScore(
    summary.approvedRevenue > 0
      ? Math.min(summary.approvedRevenue / 100, 100)
      : summary.estimatedRevenue / 200,
  );
  const conversionScore = clampScore(
    (summary.cvr ?? 0) * 1000 + summary.approvedConversions * 8,
  );
  const contentScore = clampScore(contentCount * 12 + postCount * 2);
  const riskScore = clampScore(
    summary.rejectedConversions > summary.approvedConversions ? 30 : 80,
  );
  const weighted =
    trafficScore * 0.2 +
    revenueScore * 0.3 +
  ...
  if (
    summary.pendingRevenue > summary.approvedRevenue * 2 &&
