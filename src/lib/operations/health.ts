...
export async function createOperationsHealthSnapshot(prisma: PrismaClient) {
  ...
  const [openAlerts, criticalAlerts, failedRuns, staleFreshness, activeTasks] =
    await Promise.all([
      prisma.alertIncident.count({ where: { status: "OPEN" } }),
      prisma.alertIncident.count({
        where: { status: "OPEN", severity: "CRITICAL" },
      }),
      prisma.scheduledTaskRun.count({
...
export async function createWeeklyOperationsReport(prisma: PrismaClient) {
  ...
  return prisma.weeklyOperationsReport.create({
    data: {
      ...
      recommendations: [
        failedRunCount > 0
          ? "Review failed task runs before enabling production automation."
          : "No failed scheduled runs.",
        openAlertCount > 0
          ? "Resolve or acknowledge open alerts."
