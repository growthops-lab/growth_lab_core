...
export function nextCronApproximation(cronExpression: string, from = new Date()) {
  ...
  if (minutePart.startsWith("*/") && hourPart === "*") {
    ...
    next.setMinutes(next.getMinutes() + (remainder === 0 ? step : step - remainder));
    return next;
  }

  const minute = minutePart === "*" ? base.getMinutes() + 1 : parseCronNumber(minutePart, 0);
  if (hourPart === "*") {
    const next = new Date(base);
