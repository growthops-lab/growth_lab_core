export async function withRetry<T>(operation: () => Promise<T>, retryable: (error: unknown) => boolean) {
  const enabled = process.env.GOOGLE_API_RETRY_ENABLED !== "false";
  const maxRetries = enabled ? Number(process.env.GOOGLE_API_MAX_RETRIES ?? 2) : 0;
