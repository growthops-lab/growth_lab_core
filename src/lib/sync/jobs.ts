...
import {
  syncSearchConsolePageDaily,
  syncSearchConsoleQueryDaily,
  syncSearchConsoleQueryPageDaily,
} from "@/src/lib/search-console/sync";
...
export async function runGoogleSyncJob(
  prisma: PrismaClient,
  jobId: string,
  runType: GoogleSyncRunType = GoogleSyncRunType.MANUAL,
) {
  const job = await prisma.googleSyncJob.findUniqueOrThrow({
    where: { id: jobId },
  });
  const { periodStart, periodEnd } = rangeFromDays(
    job.defaultDays,
    job.maxDays,
  );
  ...
  if (
    await hasActiveGoogleSyncLock(prisma, {
      mediaId: job.mediaId,
      googleSyncJobId: job.id,
    })
  ) {
  ...
  try {
    if (!isGoogleRealConnectionEnabled()) {
      throw new Error(
        "Google real connection is disabled. Enable GOOGLE_API_REAL_CONNECTION_ENABLED=true to run real API sync.",
      );
    }
    if (!job.targetPropertyId)
      throw new Error("Sync job has no target property.");
    if (job.jobType === GoogleSyncJobType.GA4_SITE_DAILY) {
      await syncGA4SiteDaily(prisma, {
        ga4PropertyId: job.targetPropertyId,
        periodStart,
        periodEnd,
        googleSyncRunId: run.id,
      });
    } else if (job.jobType === GoogleSyncJobType.GA4_PAGE_DAILY) {
      await syncGA4PageDaily(prisma, {
        ga4PropertyId: job.targetPropertyId,
        periodStart,
        periodEnd,
        googleSyncRunId: run.id,
      ...
      await syncSearchConsoleQueryDaily(prisma, {
        searchConsolePropertyId: job.targetPropertyId,
        periodStart,
        periodEnd,
        googleSyncRunId: run.id,
      });
    } else if (job.jobType === GoogleSyncJobType.GSC_PAGE_DAILY) {
      await syncSearchConsolePageDaily(prisma, {
        searchConsolePropertyId: job.targetPropertyId,
        periodStart,
        periodEnd,
        googleSyncRunId: run.id,
      ...
      await syncSearchConsoleQueryPageDaily(prisma, {
        searchConsolePropertyId: job.targetPropertyId,
        periodStart,
        periodEnd,
        googleSyncRunId: run.id,
    ...
    await prisma.googleSyncJob.update({
      where: { id: job.id },
      data: { lastRunAt: new Date(), status: GoogleSyncJobStatus.ACTIVE },
    });
    return run.id;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const realConnectionDisabled = message.includes(
      "Google real connection is disabled",
    );
    await prisma.googleSyncRun.update({
      where: { id: run.id },
      data: {
        status: realConnectionDisabled
          ? GoogleSyncRunStatus.CANCELLED
    ...
    await logGoogleApiError(prisma, {
      ...
      apiName:
        job.syncSource === "GA4"
          ? GoogleApiName.GA4
          : GoogleApiName.SEARCH_CONSOLE,
      endpoint: "local/google-sync-job",
      requestType: RequestType.GOOGLE_SYNC_JOB_RUN,
      errorMessage: message,
      actionRequired: isGoogleRealConnectionEnabled()
        ? GoogleApiActionRequired.NONE
        : GoogleApiActionRequired.REAUTHORIZE,
    });
    return run.id;
  }
}
