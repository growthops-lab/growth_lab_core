...
const affiliateDomains = ["px.a8.net", "a8.net", "afi-b.com", "t.afi-b.com", "accesstrade.net", "valuecommerce.com", "ck.jp.ap.valuecommerce.com"];
...
export function checkSocialPostRisk(text: string, linkUrl?: string | null) {
  ...
  for (const url of urls) {
    ...
    if (affiliateDomains.some((domain) => host === domain || host.endsWith(`.${domain}`))) {
      reasons.add("Direct affiliate link is blocked for X posts.");
    }
    if (shortDomains.some((domain) => host === domain || host.endsWith(`.${domain}`))) {
  ...
  const hasBlocked = reasonList.some((reason) => reason.includes("blocked") || reason.includes("exceeds"));
  const hasWarning = reasonList.length > 0;
  return {
    status: hasBlocked ? SocialPostCheckStatus.BLOCKED : hasWarning ? SocialPostCheckStatus.WARNING : SocialPostCheckStatus.PASSED,
    reasons: reasonList
  };
}
