import { Platform, SocialPostApprovalStatus, SocialPostQueueStatus, SocialPostSourceType } from "@prisma/client";
...
export async function enqueueManualSocialPost(input: {
  ...
  return prisma.socialPostQueue.upsert({
    ...
    create: {
      ...
      queueStatus: input.scheduledAt ? SocialPostQueueStatus.SCHEDULED : SocialPostQueueStatus.READY,
...
export async function approveSocialQueuedPost(queueId: string, reviewer = "local-admin") {
