...
export async function acquireSocialPostLock(queueId: string, ownerId: string) {
  ...
  const existing = await prisma.socialPostLock.findUnique({ where: { lockKey } });
  if (!existing) {
  ...
  const reusable = existing.releasedAt !== null || existing.expiresAt < new Date();
  if (!reusable) return false;

  await prisma.socialPostLock.update({
    where: { id: existing.id },
    data: { ownerId, acquiredAt: new Date(), heartbeatAt: new Date(), expiresAt, releasedAt: null }
  });
  return true;
}
