...
export function detectRewriteRisks(input: {
  title: string;
  contentHtml: string;
  suggestions: Array<{ afterText: string }>;
}) {
  const text = [
    input.title,
    input.contentHtml,
    ...input.suggestions.map((item) => item.afterText),
  ]
    .join("\n")
    .toLowerCase();
  ...
  for (const keyword of HIGH_RISK_KEYWORDS) {
    if (text.includes(keyword))
      findings.push({
        level: RewriteRiskLevel.HIGH,
        message: `High-risk expression detected: ${keyword}`,
      });
  }
  if (hasUnsafeHtml(input.contentHtml)) {
    findings.push({
      level: RewriteRiskLevel.CRITICAL,
      message:
        "Unsafe HTML detected and must be removed before WordPress update.",
    });
  }
  if (text.includes("px.a8.net") || text.includes("a8.net")) {
    findings.push({
      level: RewriteRiskLevel.MEDIUM,
      message:
        "Affiliate URL detected. Confirm placement is inside WordPress and not used for X direct posting.",
    });
  }
  const criticalCount = findings.filter(
    (item) => item.level === RewriteRiskLevel.CRITICAL,
  ).length;
  const highCount = findings.filter(
    (item) => item.level === RewriteRiskLevel.HIGH,
  ).length;
  const maxRiskLevel =
    criticalCount > 0
  ...
  const status =
    criticalCount > 0
      ? RewriteRiskCheckStatus.BLOCKED
      : findings.length > 0
