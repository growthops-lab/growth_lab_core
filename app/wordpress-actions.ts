...
import {
  connectionStatusForResult,
  logWordPressResult,
  WordPressClient,
} from "@/src/lib/wordpress/client";
import {
  canEncryptWordPressSecrets,
  encryptApplicationPassword,
} from "@/src/lib/wordpress/encryption";
...
async function addAllowedDomain(siteUrl: string) {
  ...
  await prisma.setting.upsert({
    ...
    create: {
      key: "ALLOWED_LINK_DOMAINS",
      value: [...domains, hostname].join(","),
    },
  });
  return hostname;
}
...
function mapWordPressStatus(raw: string): WordPressStatus {
  ...
  return allowed.includes(raw as WordPressStatus)
    ? (raw as WordPressStatus)
    : "draft";
}
...
async function uniqueLocalSlug(wordpressSiteId: string, slug: string) {
  ...
  throw new Error(
    "slug候補を生成できませんでした。別のslugを指定してください。",
  );
}
...
export async function createWordPressSite(formData: FormData) {
  ...
  if (!mockMode && !canEncryptWordPressSecrets()) {
    throw new WordPressConfigError(
      "WORDPRESS_ENCRYPTION_KEYが未設定のため、実WordPress接続は登録できません。",
    );
  }
  ...
  const encryptedPassword =
    applicationPassword && canEncryptWordPressSecrets()
      ? encryptApplicationPassword(applicationPassword)
      : null;
  ...
  const site = await prisma.wordPressSite.create({
    data: {
      ...
      allowPublish:
        data.allowPublish &&
        parseBoolean(process.env.WORDPRESS_ALLOW_PUBLISH ?? "false"),
      autoAddAllowedDomain: data.autoAddAllowedDomain,
      notes: [
        data.notes,
        isHttps(data.siteUrl)
          ? null
          : "HTTP URLです。本番運用ではHTTPSを推奨します。",
      ]
        .filter(Boolean)
        .join("\n"),
    },
...
export async function createMockWordPressArticle(formData: FormData) {
  const wordpressSiteId = z
    .string()
    .min(1)
    .parse(formData.get("wordpressSiteId"));
  ...
  const slug = await uniqueLocalSlug(
    site.id,
    createSlug(`${site.media.niche}-beginner-guide`),
  );
...
export async function createWordPressPost(formData: FormData) {
  ...
  const site = await prisma.wordPressSite.findUniqueOrThrow({
    where: { id: data.wordpressSiteId },
  });
  const slug = await uniqueLocalSlug(
    site.id,
    createSlug(data.slug || data.title),
  );
  await prisma.wordPressPost.create({
    data: {
      mediaId: site.mediaId,
  ...
  const result = await client
    ...
    .catch((error) =>
      resultFromError<WordPressPostResponse[]>(
        ...
        "GET",
        post.wordpressSite.mockMode ||
          process.env.WORDPRESS_MOCK_MODE !== "false",
      ),
    );
...
export async function syncWordPressDraft(formData: FormData) {
  ...
  if (
    post.approvalStatus !== ApprovalDecision.APPROVED &&
    post.localStatus !== WordPressLocalStatus.READY_TO_SYNC
  ) {
    throw new Error(
      "承認済み、または同期準備済みの記事だけWordPressへ同期できます。",
    );
  }
  if (
    !post.wordpressPostId &&
    post.duplicateCheckStatus !== DuplicateCheckStatus.UNIQUE
  ) {
  ...
  if (status === "publish" && !post.wordpressSite.allowPublish) {
    throw new Error(
      "Phase 2ではpublishが無効です。draftとして同期してください。",
    );
  }
  ...
  const payload = {
    ...
    categories: Array.isArray(post.categoryIds)
      ? (post.categoryIds as number[])
      : undefined,
    tags: Array.isArray(post.tagIds) ? (post.tagIds as number[]) : undefined,
  };
  const result = await client
    .createOrUpdatePost(payload, post.wordpressPostId)
    .catch((error) =>
      resultFromError<WordPressPostResponse>(
        error,
  ...
  if (result.ok && result.data) {
    await prisma.wordPressPost.update({
      ...
      data: {
        ...
        localStatus:
          result.data.status === "publish"
            ? WordPressLocalStatus.PUBLISHED
            : WordPressLocalStatus.SYNCED_DRAFT,
        wordpressPostId: String(result.data.id),
        wordpressPostUrl: result.data.link,
        wordpressEditUrl: buildEditUrl(
          post.wordpressSite.siteUrl,
          result.data.id,
        ),
    ...
    await prisma.wordPressPost.update({
      where: { id: post.id },
      data: {
        localStatus: WordPressLocalStatus.FAILED,
        lastError: result.error ?? "WordPress sync failed",
      },
    });
  }
  ...
  await logWordPressResult({
    ...
    result,
    requestType: result.mockMode
      ? RequestType.WP_MOCK_POST_CREATE
      : post.wordpressPostId
        ? RequestType.WP_POST_UPDATE
        : RequestType.WP_POST_CREATE,
    ...
    responseSummary: result.data
      ? JSON.stringify({ id: result.data.id, link: result.data.link })
      : undefined,
  });
  revalidatePath("/");
}
...
export async function syncWordPressTerms(formData: FormData) {
  ...
  await logWordPressResult({
    ...
    requestType:
      kind === "categories"
        ? RequestType.WP_CATEGORY_SYNC
        : RequestType.WP_TAG_SYNC,
...
export async function createXPostFromWordPressPost(formData: FormData) {
  ...
  if (
    account.mediaId !== wordpressPost.mediaId ||
    account.platform !== Platform.X
  ) {
    throw new Error(
      "記事と同じメディアに紐づくXアカウントを選択してください。",
    );
  }
