...
import { createArticleImprovementTask, createTaskFromSeoRecommendation } from "@/src/lib/article-improvement/tasks";
import { createContentChangeSet, createRewriteDraft, markChangeSetStatus } from "@/src/lib/rewrite/draft";
import { applyRewriteToWordPressDraft, decideRewriteApproval, requestRewriteApproval } from "@/src/lib/rewrite/workflow";
...
export async function createManualImprovementTask(formData: FormData) {
  ...
  await createArticleImprovementTask(prisma, {
    ...
    taskType: z.nativeEnum(ArticleImprovementTaskType).parse(formData.get("taskType")),
    priority: z.nativeEnum(ArticleImprovementPriority).parse(formData.get("priority")),
...
export async function createImprovementTaskFromSeoRecommendation(formData: FormData) {
  const seoRecommendationId = z.string().min(1).parse(formData.get("seoRecommendationId"));
...
export async function createImprovementTaskFromAlert(formData: FormData) {
  ...
  const alert = await prisma.alertIncident.findUniqueOrThrow({ where: { id: alertIncidentId } });
  if (!alert.mediaId) throw new Error("Alert incident is not linked to a media site.");
  await createArticleImprovementTask(prisma, {
    mediaId: alert.mediaId,
    taskTitle: `Resolve alert: ${alert.title}`,
    taskType: ArticleImprovementTaskType.COMPREHENSIVE_REWRITE,
    priority: alert.severity === "CRITICAL" ? ArticleImprovementPriority.HIGH : ArticleImprovementPriority.MEDIUM,
...
export async function createImprovementTaskFromGrowthRecommendation(formData: FormData) {
  const growthRecommendationId = z.string().min(1).parse(formData.get("growthRecommendationId"));
  const recommendation = await prisma.growthRecommendation.findUniqueOrThrow({ where: { id: growthRecommendationId } });
  await createArticleImprovementTask(prisma, {
    mediaId: recommendation.mediaId,
    taskTitle: recommendation.title,
    taskType: ArticleImprovementTaskType.COMPREHENSIVE_REWRITE,
    priority: recommendation.priority >= 70 ? ArticleImprovementPriority.HIGH : ArticleImprovementPriority.MEDIUM,
    sourceType: ImprovementSourceType.GROWTH_RECOMMENDATION,
    sourceId: recommendation.id,
    growthRecommendationId: recommendation.id,
    reason: recommendation.description,
    expectedImpact: "Improve Growth Score after human-reviewed article changes."
  });
  revalidatePath("/");
}

export async function createRewriteDraftAction(formData: FormData) {
  const taskId = z.string().min(1).parse(formData.get("articleImprovementTaskId"));
  await createRewriteDraft(prisma, taskId);
  revalidatePath("/");
}

export async function createContentChangeSetAction(formData: FormData) {
  const rewriteDraftId = z.string().min(1).parse(formData.get("rewriteDraftId"));
  await createContentChangeSet(prisma, rewriteDraftId);
  revalidatePath("/");
}

export async function updateContentChangeSetStatus(formData: FormData) {
  const contentChangeSetId = z.string().min(1).parse(formData.get("contentChangeSetId"));
...
export async function decideRewriteApprovalAction(formData: FormData) {
  const rewriteDraftId = z.string().min(1).parse(formData.get("rewriteDraftId"));
  const decision = z.nativeEnum(RewriteApprovalDecision).parse(formData.get("decision"));
  await decideRewriteApproval(prisma, { rewriteDraftId, decision, comment: String(formData.get("comment") ?? "").trim() || undefined });
  revalidatePath("/");
}

export async function applyRewriteToWordPressDraftAction(formData: FormData) {
  const rewriteDraftId = z.string().min(1).parse(formData.get("rewriteDraftId"));
  const updateMode = z.nativeEnum(WordPressRewriteUpdateMode).parse(formData.get("updateMode") ?? "CREATE_NEW_DRAFT");
