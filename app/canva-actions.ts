...
function platformForTemplate(type: CanvaTemplateType) {
  return type === CanvaTemplateType.X_POST_IMAGE
    ? Platform.X
    : Platform.WORDPRESS;
}

function jobTypeForAsset(assetType: CreativeAssetType) {
  return assetType === CreativeAssetType.X_POST_IMAGE
    ? CanvaJobType.X_POST_IMAGE
    : CanvaJobType.FEATURED_IMAGE;
}
...
export async function createCanvaTemplate(formData: FormData) {
  ...
  if (data.canvaConnectionId) {
    ...
    if (connection.mediaId !== data.mediaId) {
      throw new Error(
        "Canva connection and template must belong to the same media site.",
      );
    }
  }
...
export async function generateMockAsset(formData: FormData) {
  ...
  const assetType = z
    .nativeEnum(CreativeAssetType)
    .parse(formData.get("assetType"));
  const subtitle = String(formData.get("subtitle") ?? "").trim();
  const canvaBrandTemplateId =
    String(formData.get("canvaBrandTemplateId") ?? "").trim() || null;
  ...
  const [media, template] = await Promise.all([
    ...
    canvaBrandTemplateId
      ? prisma.canvaBrandTemplate.findUnique({
          where: { id: canvaBrandTemplateId },
        })
  ...
  if (template && template.mediaId !== mediaId) {
    throw new Error(
      "Selected Canva template belongs to a different media site.",
    );
  }
  if (
    template &&
    ((assetType === CreativeAssetType.X_POST_IMAGE &&
      template.templateType !== CanvaTemplateType.X_POST_IMAGE) ||
      (assetType === CreativeAssetType.FEATURED_IMAGE &&
        template.templateType !== CanvaTemplateType.WORDPRESS_FEATURED_IMAGE))
  ) {
    throw new Error(
      "Selected Canva template type does not match the requested asset type.",
    );
  }
  ...
  const autofill = await createMockCanvaAutofill({
    templateId:
      template?.canvaTemplateId ?? `mock-template-${assetType.toLowerCase()}`,
  ...
  if (!autofill.ok || !autofill.data) {
    await prisma.canvaDesignJob.update({
      ...
      data: {
        status: CanvaJobStatus.FAILED,
        lastError: autofill.error ?? "Canva mock autofill failed",
      },
    });
    revalidatePath("/");
    return;
  ...
  if (!exported.ok || !exported.data) {
    await prisma.canvaDesignJob.update({
      ...
      data: {
        status: CanvaJobStatus.FAILED,
        lastError: exported.error ?? "Canva mock export failed",
      },
    });
    revalidatePath("/");
    return;
...
export async function runAssetRiskCheck(formData: FormData) {
  const creativeAssetId = z
    .string()
    .min(1)
    .parse(formData.get("creativeAssetId"));
  const asset = await prisma.creativeAsset.findUniqueOrThrow({
