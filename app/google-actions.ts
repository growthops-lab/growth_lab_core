...
import { buildGoogleOAuthUrl, isGoogleRealConnectionEnabled } from "@/src/lib/google/oauth";
...
export async function disconnectGoogleConnection(formData: FormData) {
  const connectionId = z.string().min(1).parse(formData.get("googleConnectionId"));
...
export async function testGA4PropertyConnection(formData: FormData) {
  ...
  const property = await prisma.gA4Property.findUniqueOrThrow({ where: { id: ga4PropertyId } });
  const normalized = normalizeGA4PropertyId(property.propertyId);
  if (!isGoogleRealConnectionEnabled()) {
    await prisma.gA4Property.update({
      where: { id: property.id },
      data: { propertyId: normalized, connectionStatus: GooglePropertyStatus.MOCK_CONNECTED, lastError: "Google real connection is disabled." }
    });
  } else if (!property.googleConnectionId) {
    await prisma.gA4Property.update({
      where: { id: property.id },
      data: { propertyId: normalized, connectionStatus: GooglePropertyStatus.FAILED, lastError: "GA4 property has no Google connection." }
    });
  } else {
    await prisma.gA4Property.update({
      where: { id: property.id },
      data: { propertyId: normalized, connectionStatus: GooglePropertyStatus.CONNECTED, lastError: null }
    });
  }
  await prisma.apiUsageLog.create({
    data: {
      ...
      method: "TEST",
      success: isGoogleRealConnectionEnabled() && Boolean(property.googleConnectionId),
...
export async function testSearchConsolePropertyConnection(formData: FormData) {
  const propertyId = z.string().min(1).parse(formData.get("searchConsolePropertyId"));
  const property = await prisma.searchConsoleProperty.findUniqueOrThrow({ where: { id: propertyId } });
...
export async function createGoogleSyncJob(formData: FormData) {
  ...
  const jobType = z.nativeEnum(GoogleSyncJobType).parse(formData.get("jobType"));
  const targetPropertyId = z.string().trim().min(1).parse(formData.get("targetPropertyId"));
  const syncSource =
    jobType === GoogleSyncJobType.GA4_SITE_DAILY || jobType === GoogleSyncJobType.GA4_PAGE_DAILY
  ...
  if (syncSource === GoogleSyncSource.GA4) {
    const property = await prisma.gA4Property.findUniqueOrThrow({ where: { id: targetPropertyId } });
    if (property.mediaId !== mediaId) throw new Error("GA4 property belongs to a different media.");
    if (googleConnectionId && property.googleConnectionId && property.googleConnectionId !== googleConnectionId) {
      throw new Error("GA4 property belongs to a different Google connection.");
    }
  } else {
    const property = await prisma.searchConsoleProperty.findUniqueOrThrow({ where: { id: targetPropertyId } });
    if (property.mediaId !== mediaId) throw new Error("Search Console property belongs to a different media.");
    if (googleConnectionId && property.googleConnectionId && property.googleConnectionId !== googleConnectionId) {
      throw new Error("Search Console property belongs to a different Google connection.");
    }
  }
  await prisma.googleSyncJob.create({
    data: {
      ...
      syncSource,
      targetPropertyId,
      defaultDays: z.coerce.number().int().min(1).max(365).catch(28).parse(formData.get("defaultDays")),
      maxDays: z.coerce.number().int().min(1).max(365).catch(90).parse(formData.get("maxDays")),
...
export async function pauseGoogleSyncJob(formData: FormData) {
  const jobId = z.string().min(1).parse(formData.get("googleSyncJobId"));
  await prisma.googleSyncJob.update({ where: { id: jobId }, data: { status: GoogleSyncJobStatus.PAUSED } });
  revalidatePath("/");
}

export async function resumeGoogleSyncJob(formData: FormData) {
  const jobId = z.string().min(1).parse(formData.get("googleSyncJobId"));
  await prisma.googleSyncJob.update({ where: { id: jobId }, data: { status: GoogleSyncJobStatus.ACTIVE } });
  revalidatePath("/");
}
