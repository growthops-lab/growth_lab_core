...
export async function GET(request: Request) {
  ...
  if (error)
    return NextResponse.redirect(
      new URL(`/?google_oauth_error=${encodeURIComponent(error)}`, request.url),
    );
  if (!code || !state)
    return NextResponse.redirect(
      new URL("/?google_oauth_error=missing_code_or_state", request.url),
    );
  try {
    ...
    const status = token.refresh_token
      ? GoogleConnectionStatus.CONNECTED
      : GoogleConnectionStatus.MISSING_REFRESH_TOKEN;
    await prisma.googleConnection.create({
      data: {
        ...
        lastError: token.refresh_token
          ? null
          : "Refresh token was not returned. Reauthorize with prompt=consent.",
      },
    });
    ...
    return NextResponse.redirect(
      new URL(
        `/?google_oauth_error=${encodeURIComponent(message)}`,
        request.url,
      ),
    );
  }
