...
import {
  ...
  runScheduledTask,
} from "@/src/lib/operations/runner";
...
export async function pauseScheduledTask(formData: FormData) {
  ...
  await prisma.scheduledTask.update({
    where: { id: taskId },
    data: { enabled: false, status: ScheduledTaskStatus.PAUSED },
  });
  revalidatePath("/");
}
...
export async function resumeScheduledTask(formData: FormData) {
  ...
  await prisma.scheduledTask.update({
    where: { id: taskId },
    data: { enabled: true, status: ScheduledTaskStatus.ACTIVE },
  });
  revalidatePath("/");
}
...
export async function markNotificationRead(formData: FormData) {
  const notificationId = z
    .string()
    .min(1)
    .parse(formData.get("notificationEventId"));
