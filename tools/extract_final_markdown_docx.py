...
def table_to_markdown(table: Table) -> str:
    ...
    for row in table.rows:
        rows.append([escape_table_cell(cell.text.strip()) for cell in row.cells])
    if not rows:
        return ""
...
def extract_markdown(docx_path: Path) -> str:
    ...
    for block in iter_block_items(document):
        if isinstance(block, Paragraph):
            ...
            if not in_body:
                continue
            chunks.append(paragraph_to_markdown(block.style.name, text))
...
def main() -> int:
    if len(sys.argv) not in (2, 3):
        print("Usage: extract_final_markdown_docx.py SOURCE.docx [OUT.md]", file=sys.stderr)
        return 2
