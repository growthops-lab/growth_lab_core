...
async function main() {
  ...
  await prisma.campaignBudget.create({ data: { campaignId: campaign.id, mediaId: media.id, budgetType: "CONTENT", plannedAmount: 5000 } });
  await prisma.campaignCost.create({ data: { campaignId: campaign.id, mediaId: media.id, costType: "CONTENT", actualAmount: 3000, costDate: new Date(), source: "MOCK" } });
  const item = await attachCampaignItem({ campaignId: campaign.id, itemType: CampaignItemType.WORDPRESS_POST, itemId: "smoke-item", itemTitle: "Smoke item" });
  const duplicate = await attachCampaignItem({ campaignId: campaign.id, itemType: CampaignItemType.WORDPRESS_POST, itemId: "smoke-item", itemTitle: "Smoke item" });
  await attributeRevenueToCampaign({ campaignId: campaign.id, mediaId: media.id, sourceType: "MOCK", sourceId: `smoke-revenue-${campaign.id}`, pendingRevenue: 4500, approvedRevenue: 3200 });
  const sharedSourceId = `shared-revenue-${campaign.id}`;
  await attributeRevenueToCampaign({ campaignId: campaign.id, mediaId: media.id, sourceType: "MOCK", sourceId: sharedSourceId, pendingRevenue: 1000, approvedRevenue: 800, allocationRate: 0.8 });
  ...
  await createCalendarEvent({ campaignId: campaign.id, mediaId: media.id, eventType: "SNS_POST", title: "Smoke SNS", scheduledAt: new Date(), approvalStatus: "PENDING", linkCheckStatus: "NOT_CHECKED", hasCreative: false });
  ...
  const sanitizedText = sanitizeReportText("access_token=abc123 password:super-secret");
  const sanitizedJson = sanitizeReportJson({ accessToken: "abc123", nested: { webhook: "https://example.com/hook" } });
