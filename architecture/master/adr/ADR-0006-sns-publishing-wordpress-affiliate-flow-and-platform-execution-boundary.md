# ADR-0006: SNS Publishing, WordPress Affiliate Flow, and Platform Execution Boundary
Status: Accepted
Date: 2026-07-08
Owner: Human Owner
Reviewer: Human Owner / Codex Support
Source: P1_ADR_Candidate_Triage.md
Related Index IDs: ACI-CH04-009, ACI-CH04-010, ACI-CH05-010
Related Source Candidate IDs: ADR-0029, ADR-0030, ADR-0039
Decision Type: Architecture Decision Record Draft
Target Phase: MVP / Phase 1 Implementation Preparation

---
## 1. Context
This ADR draft was selected from the First ADR Draft Wave because SNS Publishing, WordPress Affiliate Flow, and Platform Execution Boundary is likely to affect MVP and Phase 1 implementation preparation if left unresolved.
Related triage theme: SNS Publishing, WordPress Affiliate Flow, and Platform Execution Boundary.
Related source categories: SNS Platform, WordPress Platform.
...
For MVP, keep SNS publishing and WordPress affiliate traffic flow controlled by approval gates, with X used for traffic routing to owned media rather than direct affiliate link publication where platform or ASP rules restrict it.
## 6. Rationale
...
ADR-0006は、以下の条件を明記したうえでAcceptedとする。

- SNS・ASP・アフィリエイト規約確認を後続仕様に委譲する。
- MVPでは承認ゲート管理を行う。
- 詳細な規約判断は後続仕様に委譲する。

MVPでは、SNS投稿、WordPress、ASP、アフィリエイト連携に関する実行可否を無制限に自動判断しない。
MVPでは、Human ReviewとApproval Gateにより承認状態を管理する。
SNS、ASP、アフィリエイト規約の詳細判断、媒体別規約確認、ASP別運用条件、広告表記、禁止表現、投稿自動化可否の詳細は、後続の実装仕様、運用仕様、または個別ADRで確認する。
