# ADR-0002: OAuth Scope and Official API Integration Boundary
Status: Accepted
Date: 2026-07-08
Owner: Human Owner
Reviewer: Human Owner / Codex Support
Source: P1_ADR_Candidate_Triage.md
Related Index IDs: ACI-CH01-005, ACI-CH04-008, ACI-CH09-001, ACI-CH09-006, ACI-CH09-007, ACI-CH09-010, ACI-CH09-014, ACI-CH09-015
Related Source Candidate IDs: ADR-0005, ADR-0006, ADR-0028, ADR-0072, ADR-0073, ADR-0076, ADR-0080, ADR-0081
Decision Type: Architecture Decision Record Draft
Target Phase: MVP / Phase 1 Implementation Preparation

---
## 1. Context
This ADR draft was selected from the First ADR Draft Wave because OAuth Scope and Official API Integration Boundary is likely to affect MVP and Phase 1 implementation preparation if left unresolved.
Related triage theme: OAuth Scope and Official API Integration Boundary.
Related source categories: API and OAuth, Architecture Principles, SNS Platform.
...
For MVP, use official APIs only, keep OAuth scopes minimal, and require Human Owner review before any production external connection is enabled. Provider-specific scopes, webhook behavior, and token handling details must be confirmed in later implementation specifications using official provider documentation.
## 6. Rationale
